...
class EqPaySimpleDelegationContractTest(BitcoinTestFramework):
    ...
    """
        - Check that it is not possible to use an input with less value than 100 eqpay as the first input to the coinstake.
        - Check that 100 eqpay is just enough for the first input to the coinstake.
    """
    def nas_staker_must_be_100_eqpay_test(self):
        use_pos_reward = True if self.staker.getblockcount() > 5000 else False
    ...
    def run_test(self):
        ...
        print("nas_staker_must_be_100_eqpay_test")
        self.nas_staker_must_be_100_eqpay_test()
        self.sync_all()
