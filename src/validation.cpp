...
bool fAddressIndex = false; // eqpay
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        for(EqPayTransaction eqpayTransaction : eqpayTransactions){
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, eqpayTransaction.gasPrice());
            } else {
                txMinGasPrice = eqpayTransaction.gasPrice();
            }
            VersionVM v = eqpayTransaction.getVersion();
            if(v.format!=0)
            ...
            if(eqpayTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(eqpayTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(eqpayTransaction.gas() > UINT32_MAX)
            ...
            gasAllTxs += eqpayTransaction.gas();
            ...
            if(v.rootVM!=0 && (uint64_t)eqpayTransaction.gasPrice() < minGasPrice)
...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s EQPAY additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // eqpay
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // eqpay
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path eqpayDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(eqpayDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(eqpayDir.string(), std::ios::in | std::ios::out);
...
bool EqPayTxConverter::extractionEqPayTransactions(ExtractEqPayTX& eqpaytx){
    ...
    eqpaytx = std::make_pair(resultTX, resultETP);
    return true;
}
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // eqpay
    ...
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    LogPrintf("%s: new best=%s type=%s height=%d version=0x%08x log2_work=%.8g tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)%s\n", __func__,
      pindexNew->GetBlockHash().ToString(), pindexNew->IsProofOfWork() ? "PoW" : "PoS" ,pindexNew->nHeight, pindexNew->nVersion,
...
bool CChainState::ConnectTip(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // eqpay
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // eqpay
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // eqpay
            globalState->setRootUTXO(oldHashUTXORoot); // eqpay
