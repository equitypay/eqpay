...
#include <qt/forms/ui_eqrctoken.h>
...
EQRCToken::EQRCToken(const PlatformStyle *platformStyle, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::EQRCToken),
    m_model(0),
    m_clientModel(0),
    m_tokenTransactionView(0)
{
    ...
    connect(m_tokenList, &TokenListWidget::sendToken, this, &EQRCToken::on_sendToken);
    connect(m_tokenList, &TokenListWidget::receiveToken, this, &EQRCToken::on_receiveToken);
    connect(m_tokenList, &TokenListWidget::addToken, this, &EQRCToken::on_addToken);
    ...
    connect(copyTokenAddressAction, &QAction::triggered, this, &EQRCToken::copyTokenAddress);
    connect(copyTokenBalanceAction, &QAction::triggered, this, &EQRCToken::copyTokenBalance);
    connect(copyTokenNameAction, &QAction::triggered, this, &EQRCToken::copyTokenName);
    connect(copySenderAction, &QAction::triggered, this, &EQRCToken::copySenderAddress);
    connect(removeTokenAction, &QAction::triggered, this, &EQRCToken::removeToken);

    connect(m_tokenList, &TokenListWidget::customContextMenuRequested, this, &EQRCToken::contextualMenu);
}
...
void EQRCToken::setModel(WalletModel *_model)
{
    m_model = _model;
    ...
    {
        // Set current token
        connect(m_tokenList->tokenModel(), &QAbstractItemModel::dataChanged, this, &EQRCToken::on_dataChanged);
        connect(m_tokenList->tokenModel(), &QAbstractItemModel::rowsInserted, this, &EQRCToken::on_rowsInserted);
...
void EQRCToken::setClientModel(ClientModel *_clientModel)
{
...
void EQRCToken::on_goToSendTokenPage()
{
    m_sendTokenPage->show();
}

void EQRCToken::on_goToReceiveTokenPage()
{
    m_receiveTokenPage->show();
}

void EQRCToken::on_goToAddTokenPage()
{
    m_addTokenPage->show();
}

void EQRCToken::on_currentTokenChanged(QModelIndex index)
{
...
void EQRCToken::on_dataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight, const QVector<int> &roles)
{
...
void EQRCToken::on_currentChanged(QModelIndex current, QModelIndex previous)
{
    Q_UNUSED(previous);
...
void EQRCToken::on_rowsInserted(QModelIndex index, int first, int last)
{
    Q_UNUSED(index);
    Q_UNUSED(first);
    Q_UNUSED(last);
...
void EQRCToken::contextualMenu(const QPoint &point)
{
...
void EQRCToken::copyTokenAddress()
{
...
void EQRCToken::copyTokenBalance()
{
    if(indexMenu.isValid())
    {
...
void EQRCToken::copySenderAddress()
{
    if(indexMenu.isValid())
    {
...
void EQRCToken::on_sendToken(const QModelIndex &index)
{
    on_currentTokenChanged(index);
    on_goToSendTokenPage();
}

void EQRCToken::on_receiveToken(const QModelIndex &index)
{
    on_currentTokenChanged(index);
    on_goToReceiveTokenPage();
}

void EQRCToken::on_addToken()
{
    on_goToAddTokenPage();
}
