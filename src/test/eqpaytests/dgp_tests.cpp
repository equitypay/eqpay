...
#include <eqpaytests/test_utils.h>
...
void createTestContractsAndBlocks(TestChain100Setup* testChain100Setup, const valtype& code1, const valtype& code2, const valtype& code3, dev::Address addr){
    ...
    txs.push_back(createEquityPayTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, addr, 0));
    txs.push_back(createEquityPayTransaction(code1, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createEquityPayTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    auto result = executeBC(txs);

    generateBlocks(50);
    txs.clear();
    txs.push_back(createEquityPayTransaction(code2, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createEquityPayTransaction(code[4], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    result = executeBC(txs);

    generateBlocks(50);
    txs.clear();
    txs.push_back(createEquityPayTransaction(code3, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createEquityPayTransaction(code[6], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    result = executeBC(txs);
}
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test1){
    ...
    dev::eth::EVMSchedule schedule = eqpayDGP.getGasSchedule(100);
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test2){
    ...
    contractLoading();
    EquityPayDGP eqpayDGP(globalState.get());
    dev::eth::EVMSchedule schedule = eqpayDGP.getGasSchedule(0);
    BOOST_CHECK(compareEVMSchedule(schedule, dev::eth::EIP158Schedule));
}
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test3){
    ...
    dev::eth::EVMSchedule schedule = eqpayDGP.getGasSchedule(coinbaseMaturity + 900);
...
BOOST_AUTO_TEST_CASE(gas_schedule_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<EquityPayTransaction> txs;
    txs.push_back(createEquityPayTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, GasScheduleDGP, 0));
    txs.push_back(createEquityPayTransaction(code[1], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createEquityPayTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, GasScheduleDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(gas_schedule_one_paramsInstance_introductory_block_1_test2){
    ...
    EquityPayDGP eqpayDGP(globalState.get());
    int coinbaseMaturity = Params().GetConsensus().CoinbaseMaturity(0);
    dev::eth::EVMSchedule schedule = eqpayDGP.getGasSchedule(coinbaseMaturity + 2); // After initializing the tests, the height of the chain 502
...
//    initState();
    ...
    for(size_t i = 0; i < sizeList; i++){
        dev::eth::EVMSchedule schedule = eqpayDGP.getGasSchedule(i);
...
BOOST_AUTO_TEST_CASE(block_size_one_paramsInstance_introductory_block_1_test2){
    ...
    dev::h256 hashTemp(hash);
    std::vector<EquityPayTransaction> txs;
    txs.push_back(createEquityPayTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, BlockSizeDGP, 0));
    txs.push_back(createEquityPayTransaction(code[7], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createEquityPayTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, BlockSizeDGP, 0));
    auto result = executeBC(txs);

    EquityPayDGP eqpayDGP(globalState.get());
    int coinbaseMaturity = Params().GetConsensus().CoinbaseMaturity(0);
    uint32_t blockSize = eqpayDGP.getBlockSize(coinbaseMaturity + 2);
...
BOOST_AUTO_TEST_CASE(min_gas_price_default_state_test1){
    ...
    contractLoading();
    EquityPayDGP eqpayDGP(globalState.get());
    uint64_t minGasPrice = eqpayDGP.getMinGasPrice(100);
...
BOOST_AUTO_TEST_CASE(min_gas_price_default_state_test2){
    ...
    contractLoading();
    EquityPayDGP eqpayDGP(globalState.get());
    uint64_t minGasPrice = eqpayDGP.getMinGasPrice(0);
    BOOST_CHECK(minGasPrice == DEFAULT_MIN_GAS_PRICE_DGP);
}
...
BOOST_AUTO_TEST_CASE(min_gas_price_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<EquityPayTransaction> txs;
    txs.push_back(createEquityPayTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, GasPriceDGP, 0));
    txs.push_back(createEquityPayTransaction(code[10], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createEquityPayTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, GasPriceDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(min_gas_price_one_paramsInstance_introductory_block_1_test2){
    ...
    EquityPayDGP eqpayDGP(globalState.get());
    int coinbaseMaturity = Params().GetConsensus().CoinbaseMaturity(0);
    uint64_t minGasPrice = eqpayDGP.getMinGasPrice(coinbaseMaturity + 2);
...
//    initState();
    ...
    size_t sizeList = Params().GetConsensus().CoinbaseMaturity(0) + 800;
    for(size_t i = 0; i < sizeList; i++){
        uint64_t minGasPrice = eqpayDGP.getMinGasPrice(i);
